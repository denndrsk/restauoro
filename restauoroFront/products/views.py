...
def step4_filling_piece(request, project_id):
    ...
    processed_model_url = f"{settings.MEDIA_URL}/project_{project_id}/processed/processed/clean_model.stl"
    door_thickness = float(request.session.get('door_thickness', 0.1)) / 100  # Umrechnung von cm in Meter
    ...
    filling_peace_url = os.path.join('media', f'project_{project_id}/processed/filling_peace.stl')
    ...
    filling_position = (
        ...
        0,  # Y-Position bleibt 0 (falls keine weitere Logik benötigt wird)
    ...
    return render(request, 'products/step4_filling_piece.html', {
        'model_url': processed_model_url,
        'project_id': project_id,
        'filling_peace_url': filling_peace_url,
        'door_thickness': door_thickness,
        'scale_factor': scale_factor,
        'start_coordinates_x' : start_coordinates_x,
        'start_coordinates_z' : start_coordinates_z,
...
def step5_filling_peace_viewer(request,project_id):
    if request.method == 'POST':
        if 'confirm' in request.POST:
            return JsonResponse({'status': 'success', 'message': 'Wizard abgeschlossen!'})
        elif 'retry' in request.POST:
            return redirect('step1_upload')
    return render(request, 'products/step5_filling_peace_viewer.html', {'project_id': project_id})
